...
def normalization(mode):
    ...
    for i in os.listdir(raw_dir):
        ...
        for index in range(img.shape[0]-1):
            new_data[index] = img[index]
            #归一化数据
            new_data[index] = (new_data[index] - np.mean(new_data[index])) / np.std(new_data[index])
            new_data[index] = np.multiply(new_data[index],mask)
        filename = os.path.join(target_dir,i.split('.')[0])
        # new_data = new_data[:new_data.shape[0]-1]
        np.save(filename,new_data)
        print(i + ' convert success'+str(new_data.shape))
...
def convertallsmri(mode):
    raw_dir = '/home/anzeng/rhb/fmri_data/new_fmri/'+mode
    target_dir = '/home/anzeng/rhb/generate_sMRI/'+mode
    print(raw_dir)
    print(target_dir)
    mkdir(target_dir)
    for i in os.listdir(raw_dir):
        sub_path = os.path.join(raw_dir,i)
        img_data = np.load(sub_path)
        for cnt in range(img_data.shape[0]):
            filename = os.path.join(target_dir, i.split('.')[0] + '_' + str(cnt))
            data = img_data[cnt]
            np.save(filename, data)
        print('{} convert success'.format(i))
